...
class RecapData {
  ...
  String get mood {
    if (peakHour >= 22 || peakHour < 6) return 'Night Owl 🦉';
    if (peakHour < 12) return 'Morning Person ☀️';
    if (peakHour < 17) return 'Daytime Vibes 🌤️';
    return 'Evening Relaxer 🌙';
  }
}
...
class ListeningAnalytics {
  RecapData generateRecap({
    ...
    for (var e in filtered) {
      ...
      hourly[h] = HourlyStat(
        hour: h,
        playCount: hourly[h].playCount + 1,
        listeningTimeMs: hourly[h].listeningTimeMs + e.durationMs,
      );
    }
    ...
    return RecapData(
      ...
      totalTracksPlayed: filtered.length,
      uniqueTracksPlayed: uTracks.length,
      uniqueArtistsPlayed: uArtists.length,
      uniqueAlbumsPlayed: uAlbums.length,
      topTracks: _topList(filtered, totalMs, 'track'),
      topArtists: _topList(filtered, totalMs, 'artist'),
      topAlbums: _topList(filtered, totalMs, 'album'),
      topGenres: _topGenres(filtered),
      currentStreak: _streak(days, now),
      longestStreak: _longestStreak(days),
      listeningDays: days,
      hourlyActivity: hourly,
      dailyActivity: _dailyStats(filtered),
      peakHour: peakH,
      newArtistsDiscovered: discovered.length,
      discoveredArtists: discovered,
      topSessions: _sessions(filtered),
  ...
  (DateTime, DateTime) _dates(RecapPeriod p, DateTime n) {
    if (p == RecapPeriod.weekly) {
      return (
        DateTime(n.year, n.month, n.day).subtract(const Duration(days: 7)),
        DateTime(n.year, n.month, n.day),
      );
    }
    if (p == RecapPeriod.monthly) {
      return (
  ...
  List<DailyStat> _dailyStats(List<ListeningEvent> ev) {
    ...
    return m.values
        .map((d) => DailyStat(date: d.d, playCount: d.c, listeningTimeMs: d.ms))
        .toList();
  }
