...
class RecapData {
  ...
  String get periodLabel => period.name.capitalize;
...
class ListeningAnalytics {
  RecapData generateRecap({ required List<ListeningEvent> events, required RecapPeriod period, Set<String>? knownBefore }) {
    ...
    for (var e in filtered) { hourly[e.timestamp.hour] = HourlyStat(hour: e.timestamp.hour, playCount: hourly[e.timestamp.hour].playCount + 1, listeningTimeMs: hourly[e.timestamp.hour].listeningTimeMs + e.durationMs); }
  ...
  List<TopItem> _topList(List<ListeningEvent> ev, int total, String type) {
    ...
    for (var e in ev) {
      ...
      map[id]!.ms += e.durationMs; map[id]!.count++;
    }
    return map.entries.map((e) => TopItem(id: e.key, name: e.value.name, imageUrl: e.value.img, playCount: e.value.count, totalListeningTimeMs: e.value.ms, percentage: total > 0 ? (e.value.ms / total * 100) : 0))
      .sorted((a, b) => b.totalListeningTimeMs.compareTo(a.totalListeningTimeMs)).take(20).toList();
  }

  List<GenreStat> _topGenres(List<ListeningEvent> ev) {
    final m = <String, int>{}; int t = 0;
    for (var e in ev) { for (var g in e.genres) { m[g] = (m[g] ?? 0) + 1; t++; } }
    return m.entries.map((e) => GenreStat(name: e.key, count: e.value, percentage: t > 0 ? (e.value / t * 100) : 0)).sorted((a, b) => b.count.compareTo(a.count)).take(5).toList();
  }

  int _streak(List<DateTime> days, DateTime now) {
    int c = 0; var d = DateTime(now.year, now.month, now.day);
  ...
  int _longestStreak(List<DateTime> days) {
    if (days.isEmpty) return 0; int max = 1, c = 1;
    for (int i = 1; i < days.length; i++) {
      if (days[i].difference(days[i-1]).inDays == 1) { c++; max = max > c ? max : c; } else { c = 1; }
    }
    return max;
  }
  ...
  List<DailyStat> _dailyStats(List<ListeningEvent> ev) {
    final m = <String, _D>{};
    for (var e in ev) { final k = '${e.timestamp.year}-${e.timestamp.month}-${e.timestamp.day}'; m.putIfAbsent(k, () => _D(DateTime(e.timestamp.year, e.timestamp.month, e.timestamp.day))); m[k]!.c++; m[k]!.ms += e.durationMs; }
  ...
  List<SessionStat> _sessions(List<ListeningEvent> ev) {
    ...
    final s = ev.sorted((a, b) => a.timestamp.compareTo(b.timestamp));
    final res = <SessionStat>[]; var cur = s.first.timestamp; int ms = 0, cnt = 0;
    for (var e in s) {
      if (e.timestamp.difference(cur).inMinutes > 10 && cnt > 0) { res.add(SessionStat(startTime: cur, durationMinutes: ms ~/ 60000, trackCount: cnt)); cur = e.timestamp; ms = 0; cnt = 0; }
      ms += e.durationMs; cnt++; cur = e.timestamp;
    }
    if (cnt > 0) res.add(SessionStat(startTime: cur, durationMinutes: ms ~/ 60000, trackCount: cnt));
    return res.sorted((a, b) => b.durationMinutes.compareTo(a.durationMinutes)).take(3).toList();
  }

  RecapData _empty(RecapPeriod p, DateTime s, DateTime e) => RecapData(period: p, startDate: s, endDate: e, totalListeningTimeMs: 0, totalTracksPlayed: 0, uniqueTracksPlayed: 0, uniqueArtistsPlayed: 0, uniqueAlbumsPlayed: 0, topTracks: [], topArtists: [], topAlbums: [], topGenres: [], currentStreak: 0, longestStreak: 0, listeningDays: [], hourlyActivity: List.generate(24, (i) => HourlyStat(hour: i, playCount: 0, listeningTimeMs: 0)), dailyActivity: [], peakHour: 0, newArtistsDiscovered: 0, discoveredArtists: [], topSessions: [], averageSessionMinutes: 0);
}

class _Tmp { String name; String? img; int ms = 0, count = 0; _Tmp(this.name, this.img); }
class _D { DateTime d; int c = 0, ms = 0; _D(this.d); }
extension _Cap on String { String get capitalize => isEmpty ? '' : '${this[0].toUpperCase()}${substring(1)}'; }
